...
public class IngredientService : MonoBehaviour
{
    ...
    public void Initialize(GameState game, ExpeditionState expedition)
    {
        GameState = game;
    ...
    {
        var w = ExpeditionState.RarityBaseWeights;
    ...
    private bool RollChance(int time)
    {
        ...
        float chance = (float)(ExpeditionState.NextLootChance * Mathf.Pow((float)ExpeditionState.NextLootDecay, time));
    ...
    void OnEnable()
    {
        CombatEvents.OnMarkedEnemyDeath += EnemyDeathReward;
    }
    ...
    void EnemyDeathReward(EnemyInstance enemy)
    {
        if (enemy.MarkedEnemy)
        {
            ...
            for (int i = 0; i < ExpeditionState.MaxMarkedLoot; i++)
            {
                if (i > 1)
                {
                    Luck = RollChance(i);
