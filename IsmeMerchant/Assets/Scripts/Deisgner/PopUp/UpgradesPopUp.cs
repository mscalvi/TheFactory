...
{    
    ...
    public void ShowUpgrades(List<UpgradeInstance> options, Action<UpgradeInstance> onSelected, GameState GameState)
    {
        Hide();
        ...
        {
            Title.text = "Choose a Upgrade to Unlock";
        }

        if (GameState.ActualLanguage == GameState.Language.Portugues)
        {
            Title.text = "Escolha uma Melhoria para Desbloquear";
        }
