...
namespace BingoCreator.Services
{
    internal class PrintingService
    {
        ...
        {
            var cards = FinderDataBase.GetCardSetById(setId);
            if (cards == null)
            {
                return;
            }

            List<DataRow> setCards = FinderDataBase.GetCardsBySetId(cards.Id);

            List<List<ItemModel>> cardElements = FinderDataBase.GetCardElementsBySet(setCards);
        ...
        public static void PrintCards5x5(CardSetModel cards, List<List<ItemModel>> cardElements)
        {
            ...
            var drawNumbers = BuildDrawNumbers(cards);
        ...
        public static void PrintCards4x4(CardSetModel cards, List<List<ItemModel>> cardElements)
        {
            var desktop = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
            var folder = Path.Combine(desktop, Sanitize(cards.Name));
            Directory.CreateDirectory(folder);
        ...
        public static void PrintList4(CardSetModel cards)
        {
            ...
            var items = (cards.AllElements ?? new List<ItemModel>())
            ...
            bool showColumnLetterPerCell = false)
        {
            ...
            //    - 5x5: une B,I,N,G,O
            //    - 4x4: usa AllElements
            IEnumerable<ItemModel> src =
                cards.CardsSize == 5
                    ? new[]
                      {
                  cards.GroupB ?? new List<ItemModel>(),
                  cards.GroupI ?? new List<ItemModel>(),
                  cards.GroupN ?? new List<ItemModel>(),
                  cards.GroupG ?? new List<ItemModel>(),
                  cards.GroupO ?? new List<ItemModel>()
                      }.SelectMany(x => x)
                    : (cards.AllElements ?? new List<ItemModel>());
            ...
            if (cards.CardsSize == 5)
            {
                ...
                var groups = new List<(char Col, IEnumerable<ItemModel> Elems)>
    {
        (header[0], cards.GroupB ?? new List<ItemModel>()),
        (header[1], cards.GroupI ?? new List<ItemModel>()),
        (header[2], cards.GroupN ?? new List<ItemModel>()),
        (header[3], cards.GroupG ?? new List<ItemModel>()),
        (header[4], cards.GroupO ?? new List<ItemModel>())
    };
            ...
            {
                var srcAll = cards.AllElements ?? new List<ItemModel>();
        ...
        private static void DrawCards5x5(XGraphics gfx, double x, double y, double width, double height, List<ItemModel> cardsElements, int cardNumber, Dictionary<int, int> drawNumbers, string titleText, string footerText, Theme theme, XPen pen, XFont titleFont, XFont headerFont, XFont elementFont, XFont footerFont, XFont numberFont)
        {
            double padding = 8;
            ...
            for (int row = 0; row < 5; row++)
            {
                for (int col = 0; col < 5; col++)
                {
                    ...
                    if (idx < cardsElements.Count)
                    {
                        ...
                        drawNumbers.TryGetValue(el.Id, out int drawNumber);

                        // string name = $"{drawNumber}- {(el?.CardName ?? el?.Name ?? string.Empty).Trim()}";
        ...
        private static void DrawCards4x4(XGraphics gfx, double x, double y, double width, double height, List<ItemModel> cardsElements, int cardNumber, string titleText, string footerText, Theme theme, XPen pen, XFont titleFont, XFont elementFont, XFont footerFont, XFont numberFont)
        {
            double padding = 8;
        ...
        public static void PrintCardsSubset(int setId, IEnumerable<int> cardNumbers)
        {
            ...
            var allRows = FinderDataBase.GetCardsBySetId(setId);
            var rows = allRows
            ...
            var cardElements = FinderDataBase.GetCardElementsBySet(rows);
        ...
        private static Dictionary<int, int> BuildDrawNumbers(CardSetModel cards)
        {
            var result = new Dictionary<int, int>();

            int n = 1;

            foreach (var e in cards.GroupB)
                result[e.Id] = n++;

            foreach (var e in cards.GroupI)
                result[e.Id] = n++;

            foreach (var e in cards.GroupN)
                result[e.Id] = n++;

            foreach (var e in cards.GroupG)
                result[e.Id] = n++;

            foreach (var e in cards.GroupO)
                result[e.Id] = n++;
