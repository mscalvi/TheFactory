...
namespace BingoCreator.Services
{
    public class GeneratingService
    {
        ...
        public static int CreateCards(CardSetModel cards)
        {
            ...
            List<DataRow> ElementsList = FinderDataBase.GetElementsInList(cards.ListId);
            ...
            if (cards.CardsSize == 5)
            {
                ...
                int setId = CreatorDataBase.CreateCardsSet(cards);
                ...
                for (int i = 1; i <= cards.Quantity; i++)
                {
                    ...
                    {
                        CreatorDataBase.CreateCard5(cards.ListId, companyIds, i, setId);
            ...
            else if (cards.CardsSize == 4)
            {
                ...
                for (int i = 1; i <= cards.Quantity; i++)
                {
                    ...
                    {
                        CreatorDataBase.CreateCard4(cards.ListId, elementIds, i, setId);
                        allCards.Add(selected);
                    }
                }
        ...
        private static ItemModel ToElementModel(DataRow r) => new ItemModel
        {
        ...
        public static int AddCardsToSet(int setId, int howMany)
        {
            ...
            var set = FinderDataBase.GetCardSetById(setId);
            if (set == null) throw new InvalidOperationException("Set não encontrado.");

            int created = 0;
            int nextNumber = FinderDataBase.GetMaxCardNumberBySetId(setId) + 1;

            if (set.CardsSize == 5)
            {
                // pools por coluna (IDs)
                var poolB = (set.GroupB ?? new List<ItemModel>()).Select(e => e.Id).Distinct().ToList();
                var poolI = (set.GroupI ?? new List<ItemModel>()).Select(e => e.Id).Distinct().ToList();
                var poolN = (set.GroupN ?? new List<ItemModel>()).Select(e => e.Id).Distinct().ToList();
                var poolG = (set.GroupG ?? new List<ItemModel>()).Select(e => e.Id).Distinct().ToList();
                var poolO = (set.GroupO ?? new List<ItemModel>()).Select(e => e.Id).Distinct().ToList();
                ...
                var signatures = FinderDataBase.GetExistingSignatures5(setId);
                ...
                for (int k = 0; k < howMany; k++)
                {
                    ...
                    CreatorDataBase.CreateCard5(set.ListId, elementsIds, nextNumber++, setId);
                    created++;
                }
            }
            else if (set.CardsSize == 4)
            {
                ...
                var pool = (set.AllElements ?? new List<ItemModel>()).Select(e => e.Id).Distinct().ToList();
                if (pool.Count < 16)
                    throw new InvalidOperationException("O conjunto 4×4 precisa ter pelo menos 16 elementos disponíveis.");

                var signatures = FinderDataBase.GetExistingSignatures4(setId);
                ...
                for (int k = 0; k < howMany; k++)
                {
                    ...
                    CreatorDataBase.CreateCard4(set.ListId, chosen, nextNumber++, setId);
                    created++;
                }
            }
            else
            ...
            EditorDataBase.UpdateCardSetQuantity(setId, (set.Quantity) + created);
